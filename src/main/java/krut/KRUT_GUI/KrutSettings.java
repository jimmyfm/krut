...
public class KrutSettings extends javax.swing.JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8958694956694428080L;

	/**
	 * If the present KrutSettings window is properly initiated, vCMBItem holds
	 * the Record Video checkbox from the main Krut window Menu. This parameter
	 * is used for easy access in case the Record Video checkbox from the
	 * present KrutSetting window is changed.
	 */
	public javax.swing.JCheckBoxMenuItem vCBMItem;

	/**
	 * If the present KrutSettings window is properly initiated, aCMBItem holds
	 * the Record Audio checkbox from the main Krut window Menu. This parameter
	 * is used for easy access in case the Record Audio checkbox from the
	 * present KrutSetting window is changed.
	 */
	public javax.swing.JCheckBoxMenuItem aCBMItem;

	/**
	 * If the present KrutSettings window is properly initiated, mCMBItem holds
	 * the Record Mouse Pointer checkbox from the main Krut window Menu. This
	 * parameter is used for easy access in case the Record Mouse Pointer
	 * checkbox from the present KrutSetting window is changed.
	 */
	public javax.swing.JCheckBoxMenuItem mCBMItem;

	/**
	 * How often we sample for the mouse position in ms. The sampling is done in
	 * startMouseTimer.
	 */
	...
	/**
	 * This is used to store the current mouse position in startMouseTimer.
	 */
	private java.awt.Point mousePos;

	/**
	 * This is a flag to keep track of whether the capture size is being
	 * changed.
	 */
	...
	/**
	 * Times used to track the mouse position for both the mouse position
	 * window, and the capture size window
	 */
	public javax.swing.Timer mouseTimer;

	/** Creates new form KrutSettings */
	public KrutSettings(java.awt.Rectangle capRect, int startFps,
			int startEncFps, int startEncQuality, boolean startStereo,
			boolean startSixteen, int startFrequency) {

		initComponents();

		soundQuery1.init(startFrequency, startStereo, startSixteen);
		qualitySlider1.init(startEncQuality);
		fPSQuery1.init(startFps, startEncFps);
	...
	public void loadMoreSettingsFromConfiguration() {
		if (config.hasBeenLoaded()) {

			syncCheckbox.setSelected( ! config.isAudioSynched());
			syncCheckbox.doClick(); 
			
			saveEnumCheckbox.setSelected(!config.isOverWriteAllowed()); 
			saveEnumCheckbox.doClick(); 
		
			
			saveFileChooser1.videoFile = new File(config.getVideoPath());
			saveFileChooser1.audioFile = new File(config.getAudioPath());
			saveFileChooser1.imageFile = new File(config.getImagePath());
	...
	/**
	 * Change the save file names in the text fields for the save files. This is
	 * done by copying the absolute paths from the SaveFileChooser. Notice that
	 * the actual filenames are often set elsewhere (normally the checkInited()
	 * method of Run_KRUT).
	 */
	public void changeFileNames() {
		if (saveFileChooser1.videoFile != null) {
			String moviePath = saveFileChooser1.videoFile.getAbsolutePath();
			movieFile.setText(moviePath);
			config.setVideoPath(moviePath);

		}
		if (saveFileChooser1.audioFile != null) {
			String audioPath = saveFileChooser1.audioFile.getAbsolutePath();
			audioFile.setText(audioPath);
			config.setAudioPath(audioPath);
		}
		if (saveFileChooser1.imageFile != null) {
			String imagePath = saveFileChooser1.imageFile.getAbsolutePath();
			screenFile.setText(imagePath);
	...
	/**
	 * Set the Record Video checkbox of the main Krut window. This checkbox is
	 * called whenever the the Record Video checkbox in this KrutSettings window
	 * is changed.
	 * 
	 * @param vBox
	 *            The Record Video checkbox of the main Krut window.
	 */
	...
	/**
	 * Set the Record Audio checkbox of the main Krut window. This checkbox is
	 * called whenever the the Record Audio checkbox in this KrutSettings window
	 * is changed.
	 * 
	 * @param aBox
	 *            The Record Audio checkbox of the main Krut window.
	 */
	...
	/**
	 * Set the Record Mouse Pointer checkbox of the main Krut window. This
	 * checkbox is called whenever the the Record Mouse Pointer checkbox in this
	 * KrutSettings window is changed.
	 * 
	 * @param mBox
	 *            The Record Video checkbox of the main Krut window.
	 */
	 ...
	 * 
	 * @param newVal
	 *            A boolean representing the new value of the checkbox.
	 */
	public void setVideoCheckBox(boolean newVal) {
		videoOutCheckbox.setSelected(newVal);
		config.setVideoEnabled(newVal);
	}
	...
	public void setAudioCheckBox(boolean newVal) {
		...
		config.setAudioEnabled(newVal);
	}

	/**
	 * Change the value of the Record Mouse Pointer checkbox in the KrutSettings
	 * window.
	 * 
	 * @param newVal
	 *            A boolean representing the new value of the checkbox.
	...
	public void setMouseCheckBox(boolean newVal) {
		...
		config.setMouseShowing(newVal);

	}

	/**
	 * The mouse timer is the timer that checks the mouse position on the screen
	 * for the GUI, while recording isn´t running. The timer is stopped when
	 * recording is started, and started again when recording is stopped.
	 */
		...
		java.awt.event.ActionListener timerTask = new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				if (ctrlDown && capButtonPressed) {
					capSizeQuery1.updateNumbersOnly(mouseStartPos.x,
							mouseStartPos.y, mousePos.x, mousePos.y);
				}
			}
		};
	...
	private void initComponents() {
		...
		capSizePanel.setBorder(new javax.swing.border.TitledBorder(
				"Capture Area"));
		try {
		...
		jTextArea1.setBackground(javax.swing.UIManager.getDefaults().getColor(
				"TextField.inactiveBackground"));
		jTextArea1.setEditable(false);
		jTextArea1.setLineWrap(true);
		jTextArea1.setRows(5);
		jTextArea1
				.setText("To select capture area, press this mouse pointer button. Then press and hold CTRL-button at the top left corner of the capture area. Move mouse to the bottom right corner of the capture area, and release CTRL-button. To abort at any time, press ESC.");
		jTextArea1.setWrapStyleWord(true);
		jTextArea1.setBorder(new javax.swing.border.TitledBorder(
				new javax.swing.border.LineBorder(new java.awt.Color(204, 204,
						255), 1, true), " Set capture area with mouse ",
		...
		try {
			jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource(
					"/krut/KRUT_GUI/images/kurt_test4.PNG")));
		} catch (NullPointerException ne) {
			System.out.println("Image missing");
		}
		...
		mainSavePanel.setBorder(new javax.swing.border.TitledBorder(
				"Save files"));
		...
		saveEnumCheckbox
				.setToolTipText("Toggle between overwrite and enumeration");
	...
	/**
	 * The checkbox for toggling automatic audio synchronization to video has
	 * been changed by the user.
	 * 
	 * @param evt
	 *            The ActionEvent that caused the change.
	 */
	private void syncCheckboxActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_syncCheckboxActionPerformed
		if (syncCheckbox.isSelected()) {
			...
			soundQuery1.myOutput
					.out("Audio is not synchronized to system clock");
		}
		soundQuery1.myOutput.out("");
		Configuration.instance().setAudioSynched(syncCheckbox.isSelected());
	...
	private void saveEnumCheckboxActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_saveEnumCheckboxActionPerformed
		saveFileChooser1.imageFile = saveFileChooser1
				.filterFile(saveFileChooser1.imageFile);
		saveFileChooser1.audioFile = saveFileChooser1
				.filterFile(saveFileChooser1.audioFile);
		saveFileChooser1.videoFile = saveFileChooser1
				.filterFile(saveFileChooser1.videoFile);
		changeFileNames();
		...
		conf.setVideoPath(saveFileChooser1.videoFile.getAbsolutePath());
		conf.setImagePath(saveFileChooser1.imageFile.getAbsolutePath());
		conf.setAudioPath(saveFileChooser1.audioFile.getAbsolutePath());

		conf.setOverWriteAllowed(saveEnumCheckbox.isSelected());
	...
	private void screenFileActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_screenFileActionPerformed
		saveFileChooser1.imageFile = saveFileChooser1
				.filterFile(new java.io.File(screenFile.getText()));
		changeFileNames();
		saveFileChooser1.myOutput.out("New screenshot file: "
				+ saveFileChooser1.imageFile.getAbsolutePath());
		saveFileChooser1.myOutput.out("");
	...
	private void audioFileActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_audioFileActionPerformed
		saveFileChooser1.audioFile = saveFileChooser1
				.filterFile(new java.io.File(audioFile.getText()));
		changeFileNames();
		saveFileChooser1.myOutput.out("New audio file: "
				+ saveFileChooser1.audioFile.getAbsolutePath());
		saveFileChooser1.myOutput.out("");
	...
	private void movieFileActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_movieFileActionPerformed
		saveFileChooser1.videoFile = saveFileChooser1
				.filterFile(new java.io.File(movieFile.getText()));
		changeFileNames();
		saveFileChooser1.myOutput.out("New video file: "
				+ saveFileChooser1.videoFile.getAbsolutePath());
		saveFileChooser1.myOutput.out("");
	...
	/**
	 * The focus of the button for changing the capture area has been lost. If
	 * the button had been pressed, and the capture area had not yet been
	 * changed by the user, the current method resets the capture area
	 * parameters to their previous values, and issues a warning to the user
	 * through the output window.
	 * 
	 * @param evt
	 *            The ActionEvent that caused the focus loss.
	 */
	private void jButton1FocusLost(java.awt.event.FocusEvent evt) {// GEN-FIRST:event_jButton1FocusLost
		if (capButtonPressed) {
			capSizeQuery1.myOutput
					.out("Focus lost, changing capture area aborted");
	...
	/**
	 * The button for changing the capture area has been pressed. This can
	 * happen in two ways: The user has directly pressed the button in the
	 * present KrutSettings window, or (more commonly) the user has pressed the
	 * mouse pointer button in the main Krut window, and that button has
	 * subsequently fired this action event.
	 * 
	 * @param evt
	 *            The ActionEvent.
	 */
	private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButton1ActionPerformed
		...
		capSizeQuery1.myOutput
				.out("To change the capture area, now press CTRL");
		capSizeQuery1.myOutput
				.out("at the top left corner of the new capture area");
		capSizeQuery1.myOutput.out("");
	}// GEN-LAST:event_jButton1ActionPerformed

	/**
	 * A key on the keyboard has been released while the button for changing the
	 * capture area is in focus. The present method checks if the key was the
	 * CTRL key, in which case the capture area is changed through a call to
	 * capSizeQuery1.
	 * 
	 * @param evt
	 *            The ActionEvent.
	...
	private void jButton1KeyReleased(java.awt.event.KeyEvent evt) {// GEN-FIRST:event_jButton1KeyReleased
		if (capButtonPressed && (evt.getKeyCode() == KeyEvent.VK_CONTROL)) {
			ctrlDown = false;
	...
	/**
	 * A key on the keyboard has been pressed while the button for changing the
	 * capture area is in focus.
	 * 
	 * The present method checks if the key was the CTRL key, in which case the
	 * ctrlDown parameter is set to true. This will make the MouseTimer start
	 * updating the capture area text fields.
	 * 
	 * The present method also checks if the pressed key was the ESC key, in
	 * which case the changing of capture size is aborted.
	 * 
	 * @param evt
	 *            The ActionEvent.
	 */
	private void jButton1KeyPressed(java.awt.event.KeyEvent evt) {// GEN-FIRST:event_jButton1KeyPressed
		if (capButtonPressed && (!ctrlDown)
			...
			ctrlDown = true;
			capSizeQuery1.myOutput
					.out("Now move the mouse pointer to the bottom right corner");
			capSizeQuery1.myOutput
					.out("of the new capture area, and release control button");
			capSizeQuery1.myOutput.out("");
	...
	/**
	 * The Record Audio checkbox has been altered. The present method simply
	 * clicks the corresponding button in the main Krut window Menu.
	 * 
	 * @param evt
	 *            The ActionEvent that caused the change.
	 */
	...
	/**
	 * The Record Mouse Position checkbox has been altered. The present method
	 * simply clicks the corresponding button in the main Krut window Menu.
	 * 
	 * @param evt
	 *            The ActionEvent that caused the change.
	 */
	...
	/**
	 * The Record Video Checkbox has been altered. The present method simply
	 * clicks the corresponding button in the main Krut window Menu.
	 * 
	 * @param evt
	 *            The ActionEvent that caused the change.
